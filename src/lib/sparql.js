...
SELECT ?sebelum ?namaSebelum ?sesudah ?namaSesudah WHERE {
  ...
  return {
    ...
    spesifik: firstSpesifik ? {
      ...
      namaKontrolUnit: val(firstSpesifik, 'namaKontrolUnit'),
      kelompok: collectSpesifik('kelompok'),
      penjaminan: collectSpesifik('penjaminan'),
      namaRS: val(firstSpesifik, 'namaRS'),
    } : null,
    risiko: {
      umum:   dRisiko.rows.filter(r => val(r, 'tipeRisiko') === 'RisikoUmum').map(r => ({ nama: val(r, 'namaRisiko'), deskripsi: val(r, 'deskripsiRisiko') })),
      jarang: dRisiko.rows.filter(r => val(r, 'tipeRisiko') === 'RisikoJarang').map(r => ({ nama: val(r, 'namaRisiko'), deskripsi: val(r, 'deskripsiRisiko') })),
    },
